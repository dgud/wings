...
%%  wpc_thebounty.erl
%%
%%     TheBounty Plugin User Interface.
%%
%%  Copyright (c) 2003-2008 Raimo Niskanen
%%                2013-2015 Code Convertion from Yafray to TheBounty by Bernard Oortman (Wings3d user oort)
%%                2015 Micheus (porting to use wx dialogs)
%%                2020 Code copied from TheBounty to TheBounty since it supports SSS by Micheus
%%
...
-define(DEF_RENDERER, "yararay-xml").
...
material_dialog(_Name, Mat) ->
    ...
    TheBounty = proplists:get_value(?TAG, Mat, []),
    DefShaderType = get_pref(shader_type, TheBounty),
    ShaderType = proplists:get_value(shader_type, TheBounty, DefShaderType),
    Object_Type = proplists:get_value(object_type, TheBounty, ?DEF_OBJECT_TYPE),
    Volume_Type = proplists:get_value(volume_type, TheBounty, ?DEF_VOLUME_TYPE),
    Volume_Sigma_a = proplists:get_value(volume_sigma_a, TheBounty, ?DEF_VOLUME_SIGMA_A),
    Volume_Sigma_s = proplists:get_value(volume_sigma_s, TheBounty, ?DEF_VOLUME_SIGMA_S),
    Volume_Height = proplists:get_value(volume_height, TheBounty, ?DEF_VOLUME_HEIGHT),
    Volume_Steepness = proplists:get_value(volume_steepness, TheBounty, ?DEF_VOLUME_STEEPNESS),
    Volume_Attgridscale = proplists:get_value(volume_attgridscale, TheBounty, ?DEF_VOLUME_ATTGRIDSCALE),
    Volume_Sharpness = proplists:get_value(volume_sharpness, TheBounty, ?DEF_VOLUME_SHARPNESS),
    Volume_Cover = proplists:get_value(volume_cover, TheBounty, ?DEF_VOLUME_COVER),
    Volume_Density = proplists:get_value(volume_density, TheBounty, ?DEF_VOLUME_DENSITY),
    Volume_Minmax_X = proplists:get_value(volume_minmax_x, TheBounty, ?DEF_VOLUME_MINMAX_X),
    Volume_Minmax_Y = proplists:get_value(volume_minmax_y, TheBounty, ?DEF_VOLUME_MINMAX_Y),
    Volume_Minmax_Z = proplists:get_value(volume_minmax_z, TheBounty, ?DEF_VOLUME_MINMAX_Z),
    Lightportal_Power = proplists:get_value(lightportal_power, TheBounty, ?DEF_LIGHTPORTAL_POWER),
    Lightportal_Samples = proplists:get_value(lightportal_samples, TheBounty, ?DEF_LIGHTPORTAL_SAMPLES),
    Lightportal_Diffusephotons = proplists:get_value(lightportal_diffusephotons, TheBounty, ?DEF_LIGHTPORTAL_DIFFUSEPHOTONS),
    Lightportal_Causticphotons = proplists:get_value(lightportal_causticphotons, TheBounty, ?DEF_LIGHTPORTAL_CAUSTICPHOTONS),
    Lightportal_Photon_Only = proplists:get_value(lightportal_photon_only, TheBounty, ?DEF_LIGHTPORTAL_PHOTON_ONLY),
    Meshlight_Power = proplists:get_value(meshlight_power, TheBounty, ?DEF_MESHLIGHT_POWER),
    Meshlight_Samples = proplists:get_value(meshlight_samples, TheBounty, ?DEF_MESHLIGHT_SAMPLES),
    Meshlight_Color = proplists:get_value(meshlight_color, TheBounty, ?DEF_MESHLIGHT_COLOR),
    Meshlight_Double_Sided = proplists:get_value(meshlight_double_sided, TheBounty, ?DEF_MESHLIGHT_DOUBLE_SIDED),
    TIR = proplists:get_value(tir, TheBounty, ?DEF_TIR),
    AutosmoothAngle = proplists:get_value(autosmooth_angle, TheBounty, ?DEF_AUTOSMOOTH_ANGLE),
    ...
    %%
    IOR = proplists:get_value(ior, TheBounty, ?DEF_IOR),

    %% Color Properties Transmitted = Diffuse and Refracted
    %% Color Properties Reflected = Glossy and Reflected
    %%
    Reflected = proplists:get_value(reflected, TheBounty, DefReflected),
    Transmitted = proplists:get_value(transmitted, TheBounty, DefTransmitted),

    %% Glass Properties
    %%
    AbsorptionColor = proplists:get_value(absorption_color, TheBounty, DefAbsorptionColor),
    AbsorptionDist = proplists:get_value(absorption_dist, TheBounty, ?DEF_ABSORPTION_DIST),
    ...
    FakeShadows = proplists:get_value(fake_shadows, TheBounty, ?DEF_FAKE_SHADOWS),
    Roughness = proplists:get_value(roughness, TheBounty, ?DEF_ROUGHNESS),
    Glass_IR_Depth = proplists:get_value(glass_ir_depth, TheBounty, ?DEF_GLASS_IR_DEPTH),

    %% Shiny Diffuse Properties
    %% Transmit Filter also for Glass and Rough Glass
    Transparency = proplists:get_value(transparency, TheBounty, ?DEF_TRANSPARENCY),
    TransmitFilter = proplists:get_value(transmit_filter, TheBounty, ?DEF_TRANSMIT_FILTER),
    Translucency = proplists:get_value(translucency, TheBounty, ?DEF_TRANSLUCENCY),
    SpecularReflect = proplists:get_value(specular_reflect, TheBounty, ?DEF_SPECULAR_REFLECT),
    Emit = proplists:get_value(emit, TheBounty, ?DEF_EMIT),

    %% Translucency (SSS) Properties
    %%
    SSS_AbsorptionColor = proplists:get_value(sss_absorption_color, TheBounty, ?DEF_SSS_ABSORPTION_COLOR),
    ScatterColor = proplists:get_value(scatter_color, TheBounty, ?DEF_SCATTER_COLOR),
    SigmaSfactor = proplists:get_value(sigmas_factor, TheBounty, ?DEF_SIGMAS_FACTOR),
    SSS_Translucency = proplists:get_value(sss_translucency, TheBounty, ?DEF_SSS_TRANSLUCENCY),
    SSS_Specular_Color = proplists:get_value(sss_specular_color, TheBounty, ?DEF_SSS_SPECULAR_COLOR),

    %% Shiny Diffuse, Glossy, Coated Glossy Properties
    %%
    DiffuseReflect = proplists:get_value(diffuse_reflect, TheBounty, ?DEF_DIFFUSE_REFLECT),
    OrenNayar = proplists:get_value(oren_nayar, TheBounty, ?DEF_OREN_NAYAR),
    OrenNayar_Sigma = proplists:get_value(oren_nayar_sigma, TheBounty, ?DEF_OREN_NAYAR_SIGMA),

    %% Glossy and Coated Glossy Properties
    %%
    GlossyReflect = proplists:get_value(glossy_reflect, TheBounty, ?DEF_GLOSSY_REFLECT),
    Exponent = proplists:get_value(exponent, TheBounty, ?DEF_EXPONENT),
    Anisotropic = proplists:get_value(anisotropic, TheBounty, ?DEF_ANISOTROPIC),
    Anisotropic_U = proplists:get_value(anisotropic_u, TheBounty, ?DEF_ANISOTROPIC_U),
    Anisotropic_V = proplists:get_value(anisotropic_v, TheBounty, ?DEF_ANISOTROPIC_V),

    %% Light Material Properties
    %%
    Lightmat_Color = proplists:get_value(lightmat_color, TheBounty, DefLightmatColor),
    Lightmat_Power = proplists:get_value(lightmat_power, TheBounty, ?DEF_LIGHTMAT_POWER),

    %% Blend Material Properties
    %%
    Blend_Mat1 = proplists:get_value(blend_mat1, TheBounty, ?DEF_BLEND_MAT1),
    Blend_Mat2 = proplists:get_value(blend_mat2, TheBounty, ?DEF_BLEND_MAT2),
...
%                    wings_dialog:update(?KEY(pnl_shader_l), Store),
    ...
    %%
    Modulators = proplists:get_value(modulators, TheBounty, def_modulators(Maps)),
    ObjectFrame =
        {vframe, [
            {hframe, [
...
light_dialog(Name, Ps) ->
    ...
    TheBounty = proplists:get_value(?TAG, Ps, []),
    ...
    Power = proplists:get_value(power, TheBounty, DefPower),
    PowerStr =
        [{hframe, [
            {hframe, [
                {label,?__(1,"Power")},
    ...
    {vframe, PowerStr ++ light_dialog(Name, Type, TheBounty)}.
...
export_shader(F, Name, Mat, ExportDir) ->
    ...
    DefShaderType = get_pref(shader_type, TheBounty),
    ShaderType =
        proplists:get_value(shader_type, TheBounty, DefShaderType),

    case ShaderType of

        shinydiffuse ->
            export_shinydiffuse_shader(F, Name, Mat, ExportDir, TheBounty);
        glossy ->
            export_glossy_shader(F, Name, Mat, ExportDir, TheBounty);
        coatedglossy ->
            export_coatedglossy_shader(F, Name, Mat, ExportDir, TheBounty);

        translucent ->
            export_translucent_shader(F, Name, Mat, ExportDir, TheBounty);

        glass ->
            export_glass_shader(F, Name, Mat, ExportDir, TheBounty);

        lightmat ->
            export_lightmat_shader(F, Name, Mat, ExportDir, TheBounty);

        rough_glass ->
            export_rough_glass_shader(F, Name, Mat, ExportDir, TheBounty);
...
export_shinydiffuse_shader(F, Name, Mat, ExportDir, TheBounty) ->
    ...
    export_rgb(F, mirror_color,
               proplists:get_value(reflected, TheBounty, DefReflected)),
    export_rgb(F, color,
               proplists:get_value(transmitted, TheBounty, DefTransmitted)),
    ...
    case AbsorptionColor of
        ...
        {AbsR,AbsG,AbsB} ->
            AbsD =
                proplists:get_value(absorption_dist, TheBounty,
                                    ?DEF_ABSORPTION_DIST),
    ...
    case OrenNayar of
        false -> ok;
        _ ->
            OrenNayarSigma = proplists:get_value(oren_nayar_sigma, TheBounty, ?DEF_OREN_NAYAR_SIGMA),
...
export_glossy_shader(F, Name, Mat, ExportDir, TheBounty) ->
    ...
    AbsorptionColor =
        proplists:get_value(absorption_color, TheBounty, DefAbsorptionColor),
    ...
    case DispersionPower of
        ...
        _   ->
            ...
            DispersionJitter =
                proplists:get_value(dispersion_jitter, TheBounty,
    ...
    case OrenNayar of
        false -> ok;
        _ ->
            OrenNayarSigma = proplists:get_value(oren_nayar_sigma, TheBounty,
...
export_translucent_shader(F, Name, Mat, ExportDir, TheBounty) ->
    ...
    SSS_AbsorptionColor =
        proplists:get_value(sss_absorption_color, TheBounty, ?DEF_SSS_ABSORPTION_COLOR),



    ScatterColor =
        proplists:get_value(scatter_color, TheBounty, ?DEF_SCATTER_COLOR),

    SSS_Specular_Color =
        proplists:get_value(sss_specular_color, TheBounty, ?DEF_SSS_SPECULAR_COLOR),
    ...
    export_rgb(F, specular_color,
               proplists:get_value(sss_specular_color, TheBounty, SSS_Specular_Color)),
    ...
    export_rgb(F, sigmaS,
               proplists:get_value(scatter_color, TheBounty, ScatterColor)),
...
export_glass_shader(F, Name, Mat, ExportDir, TheBounty) ->
    ...
    case AbsorptionColor of
        ...
        {_AbsR,_AbsG,_AbsB} ->
            ...
            export_rgb(F, absorption,
                       proplists:get_value(absorption_color, TheBounty, AbsorptionColor)),
    ...
    FakeShadows =
        proplists:get_value(fake_shadows, TheBounty, ?DEF_FAKE_SHADOWS),
...
export_lightmat_shader(F, Name, Mat, ExportDir, TheBounty) ->
    ...
    Lightmat_Color =
        proplists:get_value(lightmat_color, TheBounty, DefLightmatColor),
    ...
    export_rgb(F, color,
               proplists:get_value(lightmat_color, TheBounty, Lightmat_Color)),
...
export_shaderblend(F, Name, Mat, ExportDir) ->
    ...
    case ShaderType of
        ...
        blend_mat ->
            export_blend_mat_shader(F, Name, Mat, ExportDir, TheBounty);
...
%%% images file type used by TheBounty
%%% http://www.TheBounty.org/documentation/userguide/textureinput#imageinput
...
export_object_1(F, NameStr, Mesh0=#e3d_mesh{he=He0}, DefaultMaterial, MatPs, Id) ->
    TheBounty = proplists:get_value(?TAG, MatPs, []),
    _OpenGL = proplists:get_value(opengl, MatPs),
    UseHardness = proplists:get_value(use_hardness, TheBounty, ?DEF_USE_HARDNESS),
    Object_Type = proplists:get_value(object_type, TheBounty, ?DEF_OBJECT_TYPE),
    ...
    case Object_Type of
        ...
        volume ->
            ...
            case proplists:get_value(volume_type, TheBounty,
                                     ?DEF_VOLUME_TYPE) of
                uniformvolume ->
...
export_light(F, Name, Ps) ->
    case proplists:get_value(visible, Ps, true) of
        true ->
            ...
            TheBounty = proplists:get_value(?TAG, Ps, []),
            Type = proplists:get_value(type, OpenGL, []),
            export_light(F, Name, Type, OpenGL, TheBounty);
        _ ->
            undefined
    end.
...
export_light(F, Name, point, OpenGL, TheBounty) ->
    Power = proplists:get_value(power, TheBounty, ?DEF_ATTN_POWER),
    ...
    case Type of
        pointlight ->
            CastShadows =
                proplists:get_value(cast_shadows, TheBounty, ?DEF_CAST_SHADOWS),
            println(F,"       <cast_shadows bval=\"~s\"/>", [format(CastShadows)]);

        spherelight ->
            ArealightRadius =
                proplists:get_value(arealight_radius, TheBounty,
                                    ?DEF_AREALIGHT_RADIUS),
            ArealightSamples =
                proplists:get_value(arealight_samples, TheBounty,
...
export_light(F, Name, infinite, OpenGL, TheBounty) ->
    Bg = proplists:get_value(background, TheBounty, ?DEF_BACKGROUND_INFINITE),
    Type = proplists:get_value(type, TheBounty, ?DEF_INFINITE_TYPE),
    InfiniteTrue = proplists:get_value(infinite_true, TheBounty, ?DEF_INFINITE_TRUE),
    Power = proplists:get_value(power, TheBounty, ?DEF_POWER),
    Position = proplists:get_value(position, OpenGL, {0.0,0.0,0.0}),
    Diffuse = proplists:get_value(diffuse, OpenGL, {1.0,1.0,1.0,1.0}),
    SunSamples = proplists:get_value(sun_samples, TheBounty, ?DEF_SUN_SAMPLES),
    SunAngle = proplists:get_value(sun_angle, TheBounty, ?DEF_SUN_ANGLE),
    ...
    case Type of
        directional when Power > 0.0 ->
            ...
            case proplists:get_value(infinite_true, TheBounty,
                                     ?DEF_INFINITE_TRUE) of
                false ->
                    InfiniteRadius = proplists:get_value(infinite_radius, TheBounty,
                                                         ?DEF_INFINITE_RADIUS),
...
%%% Export Spot Light
export_light(F, Name, spot, OpenGL, TheBounty) ->
    Power = proplists:get_value(power, TheBounty, ?DEF_ATTN_POWER),
    Position = proplists:get_value(position, OpenGL, {0.0,0.0,0.0}),
    ...
    case Type of
        spotlight ->
            ...
            SpotPhotonOnly =
                proplists:get_value(spot_photon_only, TheBounty, ?DEF_SPOT_PHOTON_ONLY),

            SpotSoftShadows =
                proplists:get_value(spot_soft_shadows, TheBounty, ?DEF_SPOT_SOFT_SHADOWS),
            ...
            SpotBlend = proplists:get_value(spot_blend, TheBounty, ?DEF_SPOT_BLEND),
            SpotFuzzyness = proplists:get_value(spot_fuzzyness, TheBounty, ?DEF_SPOT_FUZZYNESS),
        ...
        spot_ies ->
            ...
            SpotIESFilename = proplists:get_value(spot_ies_filename, TheBounty,
...
export_light(F, Name, ambient, _OpenGL, TheBounty) ->
    Type = proplists:get_value(type, TheBounty, ?DEF_AMBIENT_TYPE),
    Power = proplists:get_value(power, TheBounty, ?DEF_POWER),
    ...
    case Type of
        hemilight when Power > 0.0 ->
            ...
            println(F, "",
                    []),
            case proplists:get_value(use_maxdistance, TheBounty,
                                     ?DEF_USE_MAXDISTANCE) of
                true ->
                    Maxdistance = proplists:get_value(maxdistance, TheBounty,
                                                      ?DEF_MAXDISTANCE),
        ...
        pathlight when Power > 0.0 ->
            ...
            UseQMC = proplists:get_value(use_QMC, TheBounty,
                                         ?DEF_USE_QMC),
            Depth = proplists:get_value(depth, TheBounty, ?DEF_DEPTH),
            CausDepth = proplists:get_value(caus_depth, TheBounty, ?DEF_CAUS_DEPTH),
            Direct = proplists:get_value(direct, TheBounty, ?DEF_DIRECT),
            Samples = proplists:get_value(samples, TheBounty,
                                          ?DEF_SAMPLES),
            ...
            case Direct of
                true ->
                    print(F, " direct=\"on\"");
                false ->
                    case proplists:get_value(cache, TheBounty, ?DEF_CACHE) of
                        true ->
                            CacheSize =
                                proplists:get_value(cache_size, TheBounty,
                                                    ?DEF_CACHE_SIZE),
                            AngleThreshold =
                                proplists:get_value(angle_threshold, TheBounty,
                                                    ?DEF_ANGLE_THRESHOLD),
                            ShadowThreshold =
                                proplists:get_value(shadow_threshold, TheBounty,
                                                    ?DEF_SHADOW_THRESHOLD),
                            Gradient =
                                proplists:get_value(gradient, TheBounty,
                                                    ?DEF_GRADIENT),
                            ShowSamples =
                                proplists:get_value(show_samples, TheBounty,
                                                    ?DEF_SHOW_SAMPLES),
                            Search =
                                proplists:get_value(search, TheBounty, ?DEF_SEARCH),
                            print(F, " cache=\"on\"~n"
                                  "       cache_size=\"~.10f\" "
            ...
            println(F, ">"),
            PathlightMode = proplists:get_value(pathlight_mode, TheBounty,
                                                ?DEF_PATHLIGHT_MODE),
...
export_light(F, Name, area, OpenGL, TheBounty) ->
    ...
    Power = proplists:get_value(power, TheBounty, ?DEF_ATTN_POWER),
    Samples = proplists:get_value(arealight_samples, TheBounty,
                                  ?DEF_AREALIGHT_SAMPLES),
    Dummy = proplists:get_value(dummy, TheBounty, ?DEF_DUMMY),
    ...
    undefined;
export_light(_F, Name, Type, _OpenGL, _TheBounty) ->
...
%% Constant Background Export
        constant ->
            ...
            AmbientDiffusePhotons = proplists:get_value(ambient_diffusephotons, TheBounty, ?DEF_AMBIENT_DIFFUSEPHOTONS),
            ...
            println(F, "<type sval=\"~s\"/>",
                [format(Bg)]),
            BgColor = proplists:get_value(background_color, TheBounty,
                ?DEF_BACKGROUND_COLOR),
...
%% Gradient Background Export
        gradientback ->
            ...
            println(F, "<type sval=\"~s\"/>",
                [format(Bg)]),
            HorizonColor = proplists:get_value(horizon_color, TheBounty,
                ?DEF_HORIZON_COLOR),
            export_rgb(F, horizon_color, HorizonColor),
            ZenithColor = proplists:get_value(zenith_color, TheBounty,
                ?DEF_ZENITH_COLOR),
            export_rgb(F, zenith_color, ZenithColor),
            GradientBackPower = proplists:get_value(power, TheBounty,
                ?DEF_POWER),
...
%% Sunsky Background Export
        sunsky ->
%%% 	    Power = proplists:get_value(power, TheBounty, ?DEF_POWER),
            Turbidity = proplists:get_value(turbidity, TheBounty, ?DEF_TURBIDITY),
            A_var = proplists:get_value(a_var, TheBounty, ?DEF_SUNSKY_VAR),
            B_var = proplists:get_value(b_var, TheBounty, ?DEF_SUNSKY_VAR),
            C_var = proplists:get_value(c_var, TheBounty, ?DEF_SUNSKY_VAR),
            D_var = proplists:get_value(d_var, TheBounty, ?DEF_SUNSKY_VAR),
            E_var = proplists:get_value(e_var, TheBounty, ?DEF_SUNSKY_VAR),

            SkyBackgroundLight = proplists:get_value(sky_background_light, TheBounty, ?DEF_SKY_BACKGROUND_LIGHT),
            SkyBackgroundSamples = proplists:get_value(sky_background_samples, TheBounty, ?DEF_SKY_BACKGROUND_SAMPLES),

            SunReal = proplists:get_value(sun_real, TheBounty, ?DEF_SUN_REAL),
            SunRealPower = proplists:get_value(sun_real_power, TheBounty, ?DEF_SUN_REAL_POWER),
...
%% Add Sun Real Start
            case proplists:get_value(sun_real, TheBounty,
                ?DEF_SUN_REAL) of
                true ->
                    SunRealPower = proplists:get_value(sun_real_power, TheBounty,
                        ?DEF_SUN_REAL_POWER),
...
%% Add Skylight Start
            ...
            case proplists:get_value(sky_background_light, TheBounty,
                ?DEF_SKY_BACKGROUND_LIGHT) of
                true ->
                    SkyBackgroundPower = proplists:get_value(sky_background_power, TheBounty,
                        ?DEF_SKY_BACKGROUND_POWER),
                    SkyBackgroundSamples = proplists:get_value(sky_background_samples, TheBounty,
...
%%% 	    Power = proplists:get_value(power, TheBounty, ?DEF_POWER),
            ...
            DarkskyAltitude = proplists:get_value(darksky_altitude, TheBounty, ?DEF_DARKSKY_ALTITUDE),
            SkyBackgroundLight = proplists:get_value(sky_background_light, TheBounty, ?DEF_SKY_BACKGROUND_LIGHT),
            SkyBackgroundSamples = proplists:get_value(sky_background_samples, TheBounty, ?DEF_SKY_BACKGROUND_SAMPLES),
            ...
            DarkskyNight = proplists:get_value(darksky_night, TheBounty, ?DEF_DARKSKY_NIGHT),
            DarkskyDiffusePhotons = proplists:get_value(darksky_diffusephotons, TheBounty, ?DEF_DARKSKY_DIFFUSEPHOTONS),
            DarkskyCausticPhotons = proplists:get_value(darksky_causticphotons, TheBounty, ?DEF_DARKSKY_CAUSTICPHOTONS),
...
%% Add Darksky Photons
            case proplists:get_value(darksky_diffusephotons, TheBounty,
            ...
            case proplists:get_value(darksky_causticphotons, TheBounty,
...
%% Add Darksky Night
            case proplists:get_value(darksky_night, TheBounty,
...
%% HDRI Background Export
        'HDRI' ->
            BgFname = proplists:get_value(background_filename_HDRI, TheBounty,
                ?DEF_BACKGROUND_FILENAME),
            BgExpAdj = proplists:get_value(power, TheBounty,
                ?DEF_POWER),
            BgMapping = proplists:get_value(background_mapping, TheBounty,
                ?DEF_BACKGROUND_MAPPING),

            BgRotation = proplists:get_value(background_rotation, TheBounty,
...
%% Image Background Export
        image ->
            BgFname = proplists:get_value(background_filename_image, TheBounty,
                ?DEF_BACKGROUND_FILENAME),
            BgPower = proplists:get_value(power, TheBounty,
                ?DEF_POWER),
            Samples = proplists:get_value(samples, TheBounty,
                ?DEF_SAMPLES),
            BgRotation = proplists:get_value(background_rotation, TheBounty,
...
export_render(F, CameraName, BackgroundName, Outfile, Attr) ->
    ...
    case Lighting_Method of
        directlighting ->
            ...
            println(F, "<bg_transp bval=\"~s\"/>",[format(SaveAlpha)]),
...
help(text, pref_dialog) ->
    [?__(34,"These are user preferences for the TheBounty exporter plugin"),
     ...
     ?__(72,"Executable: The rendering command for the TheBounty raytrace "
        ...
        "that executable."),
     %%
     ?__(73,"TheBounty Plugins Path: The path to the TheBounty raytrace renderer plugins "
        "folder('c:/TheBounty/bin/plugins'). TheBounty will not work without this."),
     %%
     ...
     ?__(75,"NOTE: The TheBounty Fork Build, 'TheBounty' by Povmaniac, is required for "
...
help(text, {options_dialog,general}) ->
    [[{bold,?__(82,"Transparent Shadows:")++" "}],
     ...
     [{bold,?__(98,"Rendering Error:")++" "}],
        ...
        "Subsurface Scattering, in the Lighting tab, when using a version of TheBounty that does not "
