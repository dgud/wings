    ...
    Dialog = [
        {vframe, [
            ...
            {label_column, [
                ...
                {?__(7,"Bounty Plugins Path"),{button,{text,PluginsPath,[{key,pluginspath},{width,35},{props,[{dialog_type,dir_dialog}]}]}}},
                {?__(5,"Options"),{text,Options,[{key,options}]}},
                {?__(8,"Default Shader"),{menu,menu_shader(), ShaderType, [{key,shader_type}]}}
            ]}
        ], [{title,""}]}],
    wpa:dialog(?__(6,"Bounty Options"), Dialog, fun (Attr) -> pref_result(Attr,St) end).
