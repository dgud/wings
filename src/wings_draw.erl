...
-export([refresh_dlists/1,
	 ...
	 split/4,original_we/1,update_dynamic/3,join/2,abort_split/2]).
...
invalidate_dlists(#st{selmode=Mode,sel=Sel}=St) ->
    ...
    wings_dl:map(fun(D0, Src, Data) ->
			 D = update_mirror(D0, Src),
			 invalidate_sel(D, Src, Data, Mode)
		 end, Sel),
    update_needed(St).

prepare_dlists(#st{shapes=Shs}) ->
    wings_dl:update(fun(D, Src, A) ->
			    prepare_fun(D, Src, A)
...
prepare_fun_1(#dlo_src{we=#we{perm=Perm0}=We0}=D, #we{perm=Perm1}=We) ->
	...
	true ->
	    ...
	    case {Perm0,Perm1} of
		{0,1} -> {keep, D#dlo_src{we=We}};
		{1,0} -> {keep, D#dlo_src{we=We}};
		_ -> prepare_fun_2(D, We)
	    end;
	false -> prepare_fun_2(D, We)
    end.

prepare_fun_2(#dlo_src{ns=Ns}=D, We) ->
    {changed, changed_we(D, #dlo_src{we=We, ns=Ns})}.
...
invalidate_by_mat(Changed0) ->
    ...
    wings_dl:map(fun(D, Src, _) -> invalidate_by_mat(D, Src, Changed) end, []).

invalidate_by_mat(#dlo{work=none,vs=none,smooth=none,proxy=false}=D, _, _) ->
    %% Nothing to do.
    D;
invalidate_by_mat(#dlo{proxy_data=Pd}=D, #dlo_src{we=We}, Changed) ->
...
invalidate_sel(D, #dlo_src{we=#we{id=Id},sel=SrcSel}=Src,
               [{Id,Items}|Sel], Mode) ->
    case SrcSel of
	{Mode,Items} -> {D,Sel};
	_ -> {D#dlo{sel=none,normals=none,src=Src#dlo_src{sel={Mode,Items}}},Sel}
    end;
invalidate_sel(D, Src, Sel, _) ->
    {D#dlo{sel=none,src=Src#dlo_src{sel=none}},Sel}.

changed_we(#dlo_src{ns={_}=Ns}, D) ->
    D#dlo_src{ns=Ns};
changed_we(#dlo_src{ns=Ns}, D) ->
    D#dlo_src{ns={Ns}}.


-spec update_normals(#dlo_src{}) -> #dlo_src{}.
update_normals(#dlo_src{ns={Ns0},we=#we{fs=Ftab}=We}=D) ->
    Ns = update_normals_1(Ns0, gb_trees:to_list(Ftab), We),
    D#dlo_src{ns=Ns};
update_normals(#dlo_src{ns=none,we=#we{fs=Ftab}=We}=D) ->
...
update_mirror(#dlo{mirror=none}=D,#dlo_src{we=#we{mirror=none}}) -> D;
update_mirror(#dlo{mirror=none}=D,#dlo_src{we=#we{fs=Ftab,mirror=Face}=We}) ->
...
update_needed_2(CommonNeed, St) ->
    ...
    wings_dl:map(fun(D, Src, _) ->
			 update_needed_fun(D, Src, CommonNeed, Wins, St)
		 end, []).

update_needed_fun(#dlo{}=D, #dlo_src{we=#we{perm=Perm}=We}, _, _, _)
  when ?IS_LIGHT(We), ?IS_VISIBLE(Perm) ->
    D#dlo{needed=[light]};
update_needed_fun(#dlo{}=D, #dlo_src{we=#we{perm=Perm}=We}, _, _, _)
  when ?IS_AREA_LIGHT(We), ?IS_VISIBLE(Perm) ->
    D#dlo{needed=[edges,work]};
update_needed_fun(#dlo{proxy=Proxy}=D, #dlo_src{we=#we{id=Id,he=Htab,pst=Pst}},
		  Need0, Wins, _) ->
...
build_dlists(St) ->
    wings_dl:map(fun(#dlo{needed=Needed}=D, Src0, S0) ->
			 Src = update_normals(Src0),
			 S = update_materials(Src, S0),
			 update_fun(D#dlo{src=Src}, Src, Needed, S)
		 end, St).

update_fun(D0, Src0, [H|T], St) ->
    #dlo{src=Src} = D = update_fun_2(H, D0, Src0, St),
    update_fun(D, Src, T, St);
update_fun(D, _, [], _) -> D.

-spec update_materials(#dlo_src{}, #st{}) -> #st{}.
update_materials(Src, St) ->
    We = original_we(Src),
    if ?IS_AREA_LIGHT(We) -> wings_light:shape_materials(We, St);
       true -> St
    end.

update_fun_2(light, D, Src, _) ->
    wings_light:update(D, Src);
update_fun_2(work, #dlo{work=none}=D0, Src, St) ->
    D  = wings_draw_setup:work(D0, Src, St),
    Dl = draw_flat_faces(D, St),
    D#dlo{work=Dl};
update_fun_2(smooth, #dlo{smooth=none,proxy=false}=D0, Src, St) ->
    D  = wings_draw_setup:smooth(D0, Src, St),
    Smooth = draw_smooth_faces(D, St),
    D#dlo{smooth=Smooth};
update_fun_2(smooth, #dlo{proxy=true}=D0, _Src, St) ->
    wings_proxy:smooth(D0,St);
update_fun_2({vertex,_PtSize}, #dlo{vs=none}=D, #dlo_src{we=We}, _) ->
    Data = vertices_f32(visible_vertices(We)),
    Unsel = vbo_draw_arrays(?GL_POINTS, Data),
    D#dlo{vs=Unsel};
update_fun_2(hard_edges, #dlo{hard=none}=D, #dlo_src{we=#we{he=Htab}=We}, _) ->
    Edges = gb_sets:to_list(wings_we:visible_edges(Htab, We)),
    Data = edges_f32(Edges, We),
    Hard = vbo_draw_arrays(?GL_LINES, Data),
    D#dlo{hard=Hard};
update_fun_2(edges, #dlo{edges=none}=D, #dlo_src{ns=Ns}, _) ->
    EdgeDl = make_edge_dl(array:sparse_to_list(Ns)),
    D#dlo{edges=EdgeDl};
update_fun_2(normals, D, Src,_) ->
    make_normals_dlist(D, Src);
update_fun_2(proxy, D, _, St) ->
    wings_proxy:update(D, St);

%%%% Check if plugins using the Pst need a draw list updated.
update_fun_2({plugin,{Plugin,{_,_}=Data}},#dlo{plugins=Pdl}=D,_,St) ->
...
update_sel(#dlo{}=D, #dlo_src{we=We}) when ?IS_LIGHT(We) -> {D,[]};
update_sel(#dlo{sel=none}=D, #dlo_src{sel={body,_}}=Src) ->
    update_sel_all(D, Src);
update_sel(#dlo{sel=none}=D,
           #dlo_src{split=none,sel={face,Faces},we=#we{fs=Ftab}}=Src) ->
	...
	true -> update_sel_all(D, Src);
	false -> update_face_sel(gb_sets:to_list(Faces), D, Src)
    end;
update_sel(#dlo{sel=none}=D, #dlo_src{sel={face,Faces}}=Src) ->
    %% We are dragging. Don't try to be tricky here.
    update_face_sel(gb_sets:to_list(Faces), D, Src);
update_sel(#dlo{sel=none}=D, #dlo_src{we=We, sel={edge,Edges}}) ->
    Data = edges_f32(gb_sets:to_list(Edges), We),
    Sel = vbo_draw_arrays(?GL_LINES, Data),
    D#dlo{sel=Sel};
update_sel(#dlo{sel=none}=D, #dlo_src{sel={vertex,Vs},we=#we{vp=Vtab0}}) ->
    ...
    D#dlo{sel=Points};
update_sel(#dlo{}=D, _) -> D.

%% Select all faces.
update_sel_all(#dlo{vab=#vab{face_vs=Vs}=Vab}=D, _Src) when Vs =/= none ->
...
update_sel_all(#dlo{}=D, #dlo_src{we=#we{fs=Ftab}}=Src) ->
    %% No vertex arrays to re-use. Rebuild from scratch.
    update_face_sel(gb_trees:keys(Ftab), D, Src).

update_face_sel(Fs0, #dlo{vab=#vab{face_vs=Vs,face_map=Map}=Vab}=D, #dlo_src{we=We})
  when Vs =/= none ->
    ...
    Sel = {call,F,Vab},
    D#dlo{sel=Sel};
update_face_sel(Fs0, #dlo{}=D, #dlo_src{we=We}=Src) ->
    Fs = wings_we:visible(Fs0, We),
    Data = update_face_sel_1(Fs, Src, <<>>),
    Sel = vbo_draw_arrays(?GL_TRIANGLES, Data),
    D#dlo{sel=Sel}.

update_face_sel_1(Fs, #dlo_src{ns=none,we=We}, Bin) ->
    update_face_sel_2(Fs, We, Bin);
update_face_sel_1(Fs, #dlo_src{ns={_},we=We}, Bin) ->
    update_face_sel_2(Fs, We, Bin);
...
unlit_face_bin(Face, #dlo_src{ns=Ns}, Bin) ->
...
split(#dlo{}=D, #dlo_src{ns={Ns}}=Src, Vs, St) ->
    split_1(D, Src#dlo_src{ns=Ns}, Vs, St);
split(D, Src, Vs, St) -> split_1(D, Src, Vs, St).

split_1(D, #dlo_src{split=#split{orig_we=#we{}=We,orig_ns=Ns}}=Src, Vs, St) ->
    split_2(D, Src#dlo_src{we=We,ns=Ns}, Vs, update_materials(Src, St));
split_1(D, Src, Vs, St) ->
    split_2(D, Src, Vs, update_materials(Src, St)).

split_2(#dlo{mirror=M,proxy=UsesProxy,needed=Needed,open=Open}=D,
        #dlo_src{sel=Sel,we=#we{fs=Ftab}=We}=Src0,
	Vs0, St) ->
    Vs = sort(Vs0),
    ...
    {Work,#dlo_src{ns=Ns}=Src,FtabStatic,FtabDyn} =
	split_faces(D, Src0, VisFtab, Faces, St),
    ...
    %% visible.
    StaticEdgeDl = make_static_edges(FtabStatic, Src),
    ...
    Pd = wings_proxy:split_proxy(D, Src, Vs, St),
    ...
    #dlo{work=Work,edges=[StaticEdgeDl],mirror=M,vs=VsDlist,
         src = Src#dlo_src{we=WeDyn, split=Split},
...
split_faces(#dlo{needed=Need}, Src0, Ftab0, Fs0, St) ->
	...
	true ->
	    ...
	    Src = split_new_normals(StaticFtab, Src0),

	    StaticPlan = wings_draw_setup:prepare(StaticFtab, Src, St),
	    Vab = wings_draw_setup:flat_faces(StaticPlan, Src),
	    Dl = draw_flat_faces(Vab, St),
	    {[Dl],Src,StaticFtab,DynFtab}
    end.

split_new_normals(Ftab, #dlo_src{ns=Ns0,we=We}=D) ->
...
make_static_edges(_Ftab, #dlo_src{ns=none}) ->
    make_edge_dl([]);
make_static_edges(Ftab, #dlo_src{ns=Ns}) ->
...
original_we(#dlo_src{split=#split{orig_we=We}}) -> We;
original_we(#dlo_src{we=We}) -> We.

%%%
%%% Updating of the dynamic part of a split dlist.
%%%

update_dynamic(#dlo{}=D0, #dlo_src{we=We}=Src0, Vtab) when ?IS_LIGHT(We) ->
    #dlo{src=Src1} = D = wings_light:update_dynamic(D0, Src0, Vtab),
    ...
    Src = changed_we(Src0, Src1),
    D#dlo{src=update_normals(Src)};
update_dynamic(#dlo{}=D0,
               #dlo_src{we=We0,split=#split{orig_st=St,static_vs=StaticVs}}=Src0,
               Vtab0) ->
    Vtab1 = keysort(1, StaticVs++Vtab0),
    Vtab = array:from_orddict(Vtab1),
    We = We0#we{vp=Vtab},
    Src1 = Src0#dlo_src{we=We},
    Src2 = changed_we(Src0, Src1),
    Src3 = update_normals(Src2),
    D1 = dynamic_faces(D0, Src3),
    D2 = dynamic_edges(D1, Src3),
    D3 = dynamic_influence(D2, Src3),
    D  = wings_proxy:update_dynamic(Vtab0, St, D3#dlo{src=Src3}),
    dynamic_vs(D, Src3).

dynamic_faces(#dlo{work=[Work|_]}=D,
              #dlo_src{split=#split{dyn_plan=DynPlan,orig_st=St}}=Src) ->
    Vab = wings_draw_setup:flat_faces(DynPlan, Src),
    Dl = draw_flat_faces(Vab, St),
    D#dlo{work=[Work,Dl],vab=none};
dynamic_faces(#dlo{work=none}=D, _) -> D.

dynamic_edges(#dlo{edges=[StaticEdge|_]}=D, #dlo_src{ns=Ns}) ->
    EdgeDl = make_edge_dl(array:sparse_to_list(Ns)),
    D#dlo{edges=[StaticEdge,EdgeDl]}.

dynamic_vs(#dlo{}=D,#dlo_src{split=#split{dyn_vs=none}}) -> D;
dynamic_vs(#dlo{vs=[Static|_]}=D,
           #dlo_src{we=#we{vp=Vtab},split=#split{dyn_vs=DynVs}}) ->
...
% added by Micheus
dynamic_influence(#dlo{}=D,
                  #dlo_src{we=#we{pst=Pst},split=#split{orig_st=St}}=Src) ->
   ...
   %% order to avoid problems when drawing if magnet mask is enabled.
    case gb_trees:lookup(wings_tweak,Pst) of
        ...
        {value, WeakData} ->
            ...
            update_fun(D#dlo{plugins=[]},Src,Needed,St)
    end.
...
abort_split(#dlo{}=D, #dlo_src{split=#split{orig_ns=Ns}}=Src) ->
    %% Restoring the original normals will probably be beneficial.
    D#dlo{src=Src#dlo_src{split=none,ns=Ns}};
abort_split(D, _) -> D.
...
join(#dlo{proxy_data=PD}=D,
     #dlo_src{we=#we{vp=Vtab0}=SrcWe,ns=Ns1,split=#split{orig_we=We0,orig_ns=Ns0}}) ->
    #we{vp=OldVtab} = We0,
    Vtab = join_update(Vtab0, OldVtab),
    We = wings_va:merge([SrcWe], We0#we{vp=Vtab}),
    Ns = join_ns(We, Ns1, Ns0),
    D#dlo{vs=none,drag=none,sel=none,
          proxy_data=wings_proxy:reset_dynamic(PD),
          src = #dlo_src{split=none,we=We,ns=Ns}}.
...
make_normals_dlist(#dlo{normals=none}=D, #dlo_src{we=We,sel={Mode,Elems}}) ->
    ...
    D#dlo{normals=Draw};
make_normals_dlist(#dlo{}=D, #dlo_src{sel=none}) -> D#dlo{normals=none};
make_normals_dlist(D, _) -> D.
