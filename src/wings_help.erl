...
about_panel(Parent, Imgs) ->
    ...
    wxSizer:add(SzrMain, TopPanel, [P0]),

    ImageFile = filename:join([wings_util:lib_dir(wings), "textures", "about_wings_art.png"]),
    Image = wxImage:new(ImageFile),
    BMImage = wxBitmap:new(Image),
    wxSizer:add(SzrMain, wxStaticBitmap:new(Panel, ?wxID_ANY, BMImage), [P0, {flag, ?wxALL bor ?wxALIGN_CENTER}, {border,3}]),
    ...
    wxSizer:add(SzrBottomCol, Left, [P0, {flag, ?wxLEFT}, B5]),
    wxSizer:add(SzrBottomCol, 0, 0, [P1, {flag, ?wxEXPAND}]),
    Rigth = wxStaticText:new(Panel, ?wxID_ANY, splash_content(bottom_right), [{style, ?wxALIGN_RIGHT}]),
    wxSizer:add(SzrBottomCol, Rigth, [P0, {flag, ?wxRIGHT}, B5]),

    wxSizer:add(SzrMain, SzrBottomCol, [P0, {border, 10}, {flag,?wxALL bor ?wxEXPAND}]),
